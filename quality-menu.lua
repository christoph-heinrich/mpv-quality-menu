...
local function video_formats_toggle()
    if #ui_callback > 0 then
        for name, message in pairs(ui_callback) do
            mp.commandv('script-message-to', name, message.video)
        end
    else
        show_menu(true)
    end
...
local function audio_formats_toggle()
    if #ui_callback > 0 then
        for name, message in pairs(ui_callback) do
            mp.commandv('script-message-to', name, message.audio)
        end
    else
        show_menu(false)
    end
...
mp.register_script_message('video_formats_get', function(url, script_name)
    local data = url_data[url]
    if data then
...
mp.register_script_message('audio_formats_get', function(url, script_name)
    local data = url_data[url]
    if data then
...
mp.register_script_message('video_format_set', function(url, format_id)
    set_format(url, format_id, url_data[url].afmt)
end)

mp.register_script_message('audio_format_set', function(url, format_id)
    set_format(url, url_data[url].vfmt, format_id)
end)

mp.register_script_message('register_ui', function(script_name, message_video, message_audio)
    ui_callback[script_name] = {
        video = message_video,
        audio = message_audio
    }
end)
